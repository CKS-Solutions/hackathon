    ...
    subgraph AWS["AWS"]
        subgraph EKS["Amazon EKS (Kubernetes)"]
            subgraph svc["Microsserviços"]
                ms_auth["ms-auth<br/>(JWT) register, login"]
                ms_video["ms-video<br/>upload, list, download"]
                ms_notify["ms-notify<br/>(email) SQS → SES"]
            end
            SQS["SQS (queue)"]
            PG[("PostgreSQL<br/>(auth)")]
            S3[("S3 + DynamoDB<br/>(vídeos + metadata)")]
            ALB["Ingress / ALB"]
        end
        TF["Terraform: VPC, EKS, ECR, RDS, S3, SQS, DynamoDB, SES, IAM"]
    end

    GA["GitHub Actions (CI/CD)<br/>Build & push Docker → ECR<br/>Commit image tags → Argo CD"]
    GA -->|"deploy (imagens + manifests)"| EKS

    ms_auth --> PG
    ms_video --> S3
    ms_video --> SQS
    ms_notify --> SQS
    SQS -.->|"evento vídeo processado"| ms_notify
    ALB --> ms_auth
    ALB --> ms_video
    ALB --> ms_notify
```
